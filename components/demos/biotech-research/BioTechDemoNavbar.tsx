...
import {
  ...
  companyInfo,
  demoNavLinks,
} from "@/lib/data/biotech-research/site";
import { tc } from "@/lib/theme-classes";
import { cn } from "@/lib/utils";

export function BioTechDemoNavbar() {
    ...
    >
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-4 xl:flex">
          ...
          <ThemeToggle />
          <Link href={biotechRoutes.contact} className={cn(tc.btnPrimary, "px-4 py-2 text-sm")}>
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex flex-col gap-1 px-4 py-4">
              <Link href={biotechRoutes.home} className="mb-2 font-heading font-bold text-foreground sm:hidden">
                {companyInfo.shortName}
              </Link>
              {demoNavLinks.map((link) => (
                <Link
              ...
              <Link href={biotechRoutes.contact} className={cn(tc.btnPrimary, "mt-2 text-center")}>
                Request Consultation
              </Link>
            </div>
          </motion.div>
