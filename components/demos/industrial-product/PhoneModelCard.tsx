"use client";

import { motion } from "framer-motion";
import { Battery, Camera, Cpu, Smartphone } from "lucide-react";
import type { PhoneModel } from "@/lib/types/industrial-product";
import { tc } from "@/lib/theme-classes";
import { cn } from "@/lib/utils";

interface PhoneModelCardProps {
  model: PhoneModel;
  index?: number;
}

export function PhoneModelCard({ model, index = 0 }: PhoneModelCardProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      transition={{ delay: index * 0.08 }}
      className={cn(
        tc.card,
        tc.cardHover,
        "relative flex flex-col overflow-hidden",
        model.highlighted && "ring-2 ring-accent/40",
      )}
    >
      {model.badge && (
        <div className="absolute right-4 top-4 z-10 rounded-full border border-accent/30 bg-accent/10 px-3 py-1 text-xs font-medium text-accent">
          {model.badge}
        </div>
      )}

      <div className="border-b border-border bg-gradient-to-br from-accent/5 to-transparent p-6">
        <p className="text-xs font-medium uppercase tracking-wider text-accent">
          {model.label}
        </p>
        <h3 className="font-heading mt-2 text-xl font-bold text-foreground">
          {model.name}
        </h3>
        <p className={cn("mt-2 text-sm", tc.body)}>{model.highlight}</p>
      </div>

      <div className="flex flex-1 flex-col p-6">
        <div className="grid gap-3 text-sm">
          <div className="flex items-center gap-2 text-foreground-secondary">
            <Battery className="h-4 w-4 shrink-0 text-accent" />
            <span>{model.battery}</span>
          </div>
          <div className="flex items-center gap-2 text-foreground-secondary">
            <Camera className="h-4 w-4 shrink-0 text-accent" />
            <span>{model.camera}</span>
          </div>
          <div className="flex items-center gap-2 text-foreground-secondary">
            <Smartphone className="h-4 w-4 shrink-0 text-accent" />
            <span>{model.display}</span>
          </div>
          <div className="flex items-center gap-2 text-foreground-secondary">
            <Cpu className="h-4 w-4 shrink-0 text-accent" />
            <span>{model.chip} · {model.storage}</span>
          </div>
        </div>
      </div>
    </motion.div>
  );
}
