...
import {
  ...
  constructionRoutes,
  demoNavLinks,
} from "@/lib/data/construction-engineering/site";
import { tc } from "@/lib/theme-classes";
import { cn } from "@/lib/utils";

export function ConstructionDemoNavbar() {
    ...
    >
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-4 xl:flex">
          ...
          <ThemeToggle />
          <Link
            href={constructionRoutes.contact}
            className={cn(tc.btnPrimary, "px-4 py-2 text-sm")}
          >
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex flex-col gap-1 px-4 py-4">
              <Link
                href={constructionRoutes.home}
                className="mb-2 font-heading font-bold text-foreground sm:hidden"
              >
                {companyInfo.shortName}
              </Link>
              {demoNavLinks.map((link) => (
